...
import { baseConnection, flash } from "./client";
import {
...
const RE_RPC_FORBIDDEN = /403|Access forbidden/i;
...
export function classifyTxError(e: unknown): {
  ...
  const raw = errText(e);
  if (RE_RPC_FORBIDDEN.test(raw)) {
    return {
      message:
        "mainnet RPC request was rejected — retry in a moment",
      upstream: false,
      stale: false,
      rejected: false,
    };
...
export async function enableOneClickTrading(args: {
  ...
  try {
    if (needs.basket) {
      rentLamports += await baseConnection.getMinimumBalanceForRentExemption(256);
    }
    if (needs.ledger) {
      rentLamports += await baseConnection.getMinimumBalanceForRentExemption(852);
    }
    if (needs.session) {
      rentLamports += await baseConnection.getMinimumBalanceForRentExemption(144);
    }
  } catch (e) {
    const { message, upstream } = classifyTxError(e);
    return stop(message, upstream);
  }
