...
use crate::math::swap::{compute_swap_asset_for_usdc, compute_virtual_reserves_e8};
use crate::state::*;

#[derive(AnchorSerialize, AnchorDeserialize)]
pub struct SwapAssetForUsdcArgs {
    pub amount_in: u64,
    pub min_amount_out: u64,
}

#[derive(Accounts, Session)]
pub struct SwapAssetForUsdc<'info> {
    #[account(mut)]
...
pub fn handler(ctx: Context<SwapAssetForUsdc>, args: SwapAssetForUsdcArgs) -> Result<()> {
    ...
    let amount_out =
        compute_swap_asset_for_usdc(args.amount_in, vx, vy, quote_state.spread_bps)?;
    ...
    user_bank.debit(&pool.asset_mint, args.amount_in)?;
    user_bank.credit(&pool.usdc_mint, amount_out)?;
